...
function coadd!(sc::SIRSCore, D::Array{Float64,3})
    ...
    # of the SIDECAR resets every frame
    ...
    for z in 1:sc.naxis3
        ...
        #= This first attempt did not work well and is commented out.
        # Reject high sigma outliers in left reference cols.
        sigrej = 4.0 # Things really have to be bad...
        trimmed = trim(lft, prop=TRIM_MED)
        μ = mean(trimmed)
        σ = std(trimmed)
        lft[(lft .< μ-sigrej*σ) .| (lft .> μ+sigrej*σ)] .= μ
        #
        # Reject high sigma outliers in right reference cols.
        trimmed = trim(rgt, prop=TRIM_MED)
        μ = mean(trimmed)
        σ = std(trimmed)
        rgt[(rgt .< μ-sigrej*σ) .| (rgt .> μ+sigrej*σ)] .= μ
        =#
